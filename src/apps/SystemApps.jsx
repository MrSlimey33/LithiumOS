...
export function TerminalApp({ playSound }) {
  const [log, setLog] = useState([
    "LITHIUM OS KERNEL v4.2.0-STABLE",
  ...
  const run = (e) => {
    if (e.key === 'Enter' && inp) {
      ...
      let res = `root@station:~$ ${inp}`;
      const cmd = inp.toLowerCase().trim();

      if (cmd === 'clear') setLog([]);
      else if (cmd === 'help') setLog([...log, res, "DIRECTIVES: clear, help, date, whoami, neofetch, sudo, top, network"]);
      else if (cmd === 'date') setLog([...log, res, new Date().toISOString()]);
      else if (cmd === 'whoami') setLog([...log, res, "id=0(root) gid=0(root) groups=0(root)"]);
      else if (cmd === 'neofetch') setLog([...log, res, "   .o+`             OS: Lithium Workstation v4.2\n  `ooo/             Host: Local Node (Spatial)\n `+oooo:            Kernel: WebMirror 3.0-LITH\n`+oooooo:           Shell: bash v5.1\n-+oooooo+:          Uptime: Persistent\n Lithium OS 4.0     Resolution: 2K Spatial\n Kernel: Web Native  CPU: 1x Visionary-X1\n UI: AeroComposite   Memory: 16.0 GB"]);
      else if (cmd.startsWith('sudo ')) setLog([...log, res, "Access denied. Remote elevation protocol required."]);
      else if (cmd === 'top') setLog([...log, res, "CPU: 12% | MEM: 1.4G/16G | LOAD: 0.04"]);
      else { try { res += `\n[OUT]: ${String(new Function('return ' + inp)())}`; } catch (err) { res += `\n[ERR]: Command not found. Future Linux Emulation PENDING.`; } setLog([...log, res]); }
      setInp('');
    }
  };

  return (
    <div className="w-full h-full bg-[#0c0c0e] text-cyan-400 font-mono text-[12px] p-6 flex flex-col relative overflow-hidden isolate">
      {/* ANALOG OVERLAY */}
      <div className="absolute inset-0 terminal-scanline opacity-[0.15] z-20 pointer-events-none" />
      <div className="absolute inset-0 bg-gradient-to-b from-cyan-400/[0.03] to-transparent pointer-events-none" />
      
      <div className="flex-1 overflow-y-auto whitespace-pre-wrap flex flex-col justify-end space-y-1 mb-4 relative z-10 scrollbar-hide">
        {log.map((l, i) => (
           <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} key={i} className={l.includes('[ERR]') ? 'text-rose-500' : l.includes('[OUT]') ? 'text-emerald-400' : ''}>
              {l}
           </motion.div>
        ))}
      </div>
      <div className="flex items-center gap-3 relative z-10 border-t border-white/5 pt-4">
         <span className="font-black text-emerald-500 tracking-tighter">root@station:~$</span>
         <input 
           autoFocus value={inp} onChange={e => setInp(e.target.value)} onKeyDown={run} 
           className="flex-1 bg-transparent outline-none text-white/90 placeholder-white/10 caret-cyan-400" 
